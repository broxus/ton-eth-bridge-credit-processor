const {Migration, DEX_CONTRACTS_PATH} = require(process.cwd()+'/scripts/utils')
...
async function main() {
  console.log(`35-deploy-test-dex-account.js`);
  const migration = new Migration();
  const keyPairs = await locklift.keys.getKeyPairs();

  const accountN = migration.load(await locklift.factory.getAccount('Wallet', DEX_CONTRACTS_PATH), 'Account' + options.owner_n);
  ...
  const dexAccountN = await locklift.factory.getContract(options.contract_name, DEX_CONTRACTS_PATH);
