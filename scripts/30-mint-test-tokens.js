const {Migration, TOKEN_CONTRACTS_PATH, Constants, EMPTY_TVM_CELL, DEX_CONTRACTS_PATH} = require(process.cwd()+'/scripts/utils');
...
async function main() {
  console.log(`30-mint-test-tokens.js`);
  const migration = new Migration();
  const [keyPair] = await locklift.keys.getKeyPairs();

  const rootOwner = migration.load(await locklift.factory.getAccount('Wallet', DEX_CONTRACTS_PATH), 'Account1');
  ...
  for (const mint of mints) {
    ...
    const account = migration.load(await locklift.factory.getAccount('Wallet', DEX_CONTRACTS_PATH), 'Account' + mint.account);
