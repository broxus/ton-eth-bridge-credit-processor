const {getRandomNonce, Migration, stringToBytesArray, afterRun, Constants, DEX_CONTRACTS_PATH} = require(process.cwd()+'/scripts/utils');
const { Command } = require('commander');
const program = new Command();

async function main() {
  console.log(`25-deploy-test-tokens.js`);
  ...
  const TokenFactoryCreateNewTokenFor = await locklift.factory.getContract('TokenFactoryCreateNewTokenFor', DEX_CONTRACTS_PATH);
  ...
  let tokens = options.tokens ? JSON.parse(options.tokens) : ['tst'];
