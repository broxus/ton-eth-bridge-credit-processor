const {Migration, TOKEN_CONTRACTS_PATH, Constants, afterRun, DEX_CONTRACTS_PATH} = require(process.cwd()+'/scripts/utils')
const { Command } = require('commander');
const program = new Command();

async function main() {
  console.log(`40-deploy-test-pair.js`);
  const migration = new Migration();
  const keyPairs = await locklift.keys.getKeyPairs();

  const account2 = migration.load(await locklift.factory.getAccount('Wallet', DEX_CONTRACTS_PATH), 'Account2');
  const dexVault = migration.load(await locklift.factory.getContract('DexVault', DEX_CONTRACTS_PATH), 'DexVault');
  const dexRoot = migration.load(await locklift.factory.getContract('DexRoot', DEX_CONTRACTS_PATH), 'DexRoot');
  ...
  for (const p of pairs) {
    ...
    const dexPairFooBar = await locklift.factory.getContract(options.contract_name, DEX_CONTRACTS_PATH);
