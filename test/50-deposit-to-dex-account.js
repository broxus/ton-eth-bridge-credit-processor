...
const {Migration, TOKEN_CONTRACTS_PATH, DEX_CONTRACTS_PATH, afterRun, EMPTY_TVM_CELL, Constants} = require(process.cwd() + '/scripts/utils');
...
console.log(`50-deposit-to-dex-account.js`);
...
describe('Check Deposit to Dex Account', async function () {
  ...
  before('Load contracts and balances', async function () {
    ...
    accountN = migration.load(await locklift.factory.getAccount('Wallet', DEX_CONTRACTS_PATH), 'Account' + options.owner_n);
