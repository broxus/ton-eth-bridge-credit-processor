...
options.ton_amount = options.ton_amount || '0';
options.layer3 = options.layer3 || EMPTY_TVM_CELL;
...
console.log(`70-credit-eth-ton.js`);
...
describe('Credit ETH-TON', async function () {
    ...
    describe('Deploy processor', async function () {
        ...
        it(`Check Processed`, async function () {
            ...
            expect(cpBalancesEnd[options.token_id]).to
                .equal('0', `Wrong CreditProcessor ${token.symbol} balance`);
            expect(cpBalancesEnd.ton).to
                .equal('0', `Wrong CreditProcessor TON balance`);
            expect(new BigNumber(userBalancesEnd.ton).minus(userBalancesStart.ton).gte(options.ton_amount)).to
                .equal(true, `Wrong User TON balance`);

            if(options.swap_type === 0) {
                expect(new BigNumber(userBalancesEnd[options.token_id]).minus(userBalancesStart[options.token_id] || 0).gte(options.token_amount)).to
                    .equal(true, `Wrong User ${token.symbol} balance`);
            } else {
                expect(new BigNumber(userBalancesEnd[options.token_id]).minus(userBalancesStart[options.token_id] || 0).toString()).to
                    .equal(options.token_amount, `Wrong User ${token.symbol} balance`);
            }
